...
import { Star, Phone, Car, X, Loader2 } from "lucide-react";
import { fetchDrivers, createDriver, updateDriver } from "@/lib/admin-api";
...
const statusColors: Record<string, string> = { available: "bg-green-100 text-green-700", on_trip: "bg-blue-100 text-blue-700", off_duty: "bg-gray-100 text-gray-600" };
const statusLabels: Record<string, string> = { available: "Musait", on_trip: "Yolda", off_duty: "Izinli" };
...
export default function DriversTab() {
  ...
  useEffect(() => {
    fetchDrivers().then(setDrivers).catch(console.error).finally(() => setLoading(false));
  }, []);
  ...
  const toggleStatus = async (d: Driver) => {
    const nextStatus = d.status === "available" ? "off_duty" : "available";
    await updateDriver(d.id, { status: nextStatus });
    setDrivers((prev) => prev.map((x) => x.id === d.id ? { ...x, status: nextStatus } : x));
    if (selected?.id === d.id) setSelected({ ...selected, status: nextStatus });
  };
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-lg font-semibold text-text">Suruculer</h2>
        <button onClick={() => setAdding(true)} className="btn-primary px-3 py-1.5 rounded-lg text-xs">+ Yeni Surucu</button>
      </div>

      {/* Status summary */}
      <div className="grid grid-cols-3 gap-3 mb-5">
        <div className="bg-green-50 border border-green-100 rounded-xl p-3 text-center"><p className="text-xl font-semibold text-green-700">{drivers.filter((d) => d.status === "available").length}</p><p className="text-[10px] text-green-600">Musait</p></div>
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 text-center"><p className="text-xl font-semibold text-blue-700">{drivers.filter((d) => d.status === "on_trip").length}</p><p className="text-[10px] text-blue-600">Yolda</p></div>
        <div className="bg-gray-50 border border-gray-100 rounded-xl p-3 text-center"><p className="text-xl font-semibold text-gray-600">{drivers.filter((d) => d.status === "off_duty").length}</p><p className="text-[10px] text-gray-500">Izinli</p></div>
      </div>

      {drivers.length === 0 && <div className="text-center py-12 text-secondary text-sm">Henuz surucu yok.</div>}
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-4">
                <div className="text-right hidden sm:block">
                  ...
                  <p className="text-[10px] text-tertiary">{d.trips} trip · {((d.monthly_earnings || 0) / 1000).toFixed(0)}k/ay</p>
                </div>
                <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${statusColors[d.status] || "bg-gray-100 text-gray-600"}`}>{statusLabels[d.status] || d.status}</span>
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          <div className="bg-card-bg rounded-2xl max-w-md w-full p-6 max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              ...
              <div className="bg-surface rounded-lg p-3 text-center"><p className="text-lg font-semibold text-text">{(selected.monthly_earnings || 0).toLocaleString("tr-TR")} TL</p><p className="text-[10px] text-secondary">Aylik Kazanc</p></div>
              <div className="bg-surface rounded-lg p-3 text-center"><p className="text-lg font-semibold text-text">%{selected.cancel_rate}</p><p className="text-[10px] text-secondary">Iptal Orani</p></div>
            </div>
            <div className="space-y-2 text-xs mb-4">
              ...
              <div className="flex justify-between"><span className="text-secondary">Arac</span><span className="text-text">{selected.vehicle}</span></div>
            ...
            <button onClick={() => toggleStatus(selected)} className={`w-full py-2 rounded-lg text-xs font-medium ${selected.status === "available" ? "bg-gray-100 text-gray-700" : "bg-green-100 text-green-700"}`}>
              {selected.status === "available" ? "Izine Cikart" : "Musait Yap"}
            </button>
          </div>
        </div>
      )}
      ...
      {adding && (
        <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setAdding(false)}>
          <div className="bg-card-bg rounded-2xl max-w-sm w-full p-6" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-sm font-semibold text-text mb-4">Yeni Surucu</h3>
            ...
            <div className="flex gap-2 mt-4">
              <button onClick={addDriver} disabled={saving} className="flex-1 btn-primary py-2 rounded-lg text-xs disabled:opacity-50">{saving ? "Kaydediliyor..." : "Kaydet"}</button>
              <button onClick={() => setAdding(false)} className="flex-1 bg-surface text-text py-2 rounded-lg text-xs">Iptal</button>
            </div>
          </div>
        </div>
      )}
